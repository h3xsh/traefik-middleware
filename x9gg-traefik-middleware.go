package h3xsh_traefik_middleware
...
type H3xshTraefikMiddleware struct {
	// Trace config
	traceEnabled       bool
...
func New(ctx context.Context, next http.Handler, config *Config, name string) (http.Handler, error) {
	...
	middleware := &H3xshTraefikMiddleware{
...
func (m *H3xshTraefikMiddleware) isKeyPairValid(keyName string, keyValue string) bool {
	if keyName == "" || keyValue == "" {
		return false
	}
...
func (m *H3xshTraefikMiddleware) GenerateTraceId() string {
	id := ksuid.New()
	return m.traceValuePrefix + id.String() + m.traceValueSuffix
}

func (m *H3xshTraefikMiddleware) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if m.traceEnabled {
